...
/* BEGIN SYNAPSE SECTION */
Synapse::Synapse(double w, uint32_t d, Neuron* to_n, uint32_t b) : weight(w), to(to_n), delay(d), branch(b) {};
/* END SYNAPSE SECTION */
                 ...
                 vector < double> & _stds) 
{
  ...
  leak_mode = leak;
  bool neuron_leak;
  vector<double> dendritic_couplings;
  vector<double> dendritic_thresholds;
  ...
  for(i = 0; i < net->sorted_node_vector.size(); i++) {
    dendritic_couplings.clear();
    dendritic_thresholds.clear();
    ...
    // Retrieve dendritic properties
    for(j = 0; j < num_dendrites; j++) {
      dendritic_couplings.push_back(node->get("D_Coupling_" + to_string(j)));
      dendritic_thresholds.push_back(node->get("D_Threshold_" + to_string(j)));
    }

    n = add_neuron(node->id, node->get("Threshold"), neuron_leak, dendritic_couplings, dendritic_thresholds);
...
void Network::apply_spike(const Spike& s, bool normalized)
{
  ...
  if (normalized && (s.value < 0 || s.value > 1)) {
    ...
    throw SRE((string) "drisp::Network::apply_spike() - value (" + buf + ") must be in [0,1].");
  }

  if (!is_valid_input_id(s.id)) {
    snprintf(buf, 24, "%d", s.id);
    throw SRE((string) "drisp::Network::apply_spike() - input_id " + buf + " is not valid");
  }
  ...
  if (inputs_from_weights) {
    if (!normalized) {
      ...
      if (index >= weights.size()) {
        ...
        es = "drisp::Network::apply_spike() - value must be between 0 and ";
        es += buf;
    ...
    if (normalized) {
      ...
      // spike in only the integer part of the
      // value when discrete is set to true

      v = (discrete) ? floor(s.value) : s.value;
    }
  }
  if (noisy_stddev != 0) v = rng.Random_Normal(v, noisy_stddev);
  events[s.time].push_back(Event{n, v, 0}); // injected spikes are on branch 0 (default)
}
...
void Network::run(double duration)
{
  ...
  if (duration < 0) throw SRE("drisp::Network::run() - duration < 0");
...
double Network::output_last_fire(int output_id) 
{
  ...
  if (!is_valid_output_id(output_id)) {
    snprintf(buf, 100, "drisp::Network::output_last_fire() - output_id %u is not valid", output_id);
...
int Network::output_count(int output_id) 
{
  char buf[200];
  if (!is_valid_output_id(output_id)) {
    snprintf(buf, 200, "drisp::Network::output_count() - output id %u is not valid", output_id);
    throw SRE((string) buf);
  }
...
vector <double> Network::output_vector(int output_id) 
{
  char buf[100];
  if (!is_valid_output_id(output_id)) {
    snprintf(buf, 100, "drisp::Network::output_last_vector() - output_id %u is not valid", 
             output_id);
    throw SRE((string) buf);
  }
...
Neuron* Network::add_neuron(uint32_t node_id, double threshold, bool leak, const vector<double>& dendritic_couplings, const vector<double>& dendritic_thresholds)
{
  Neuron *n;
  char buf[200];

  if (is_neuron(node_id)) {
    snprintf(buf, 200, "drisp::Neuron::add_neuron() - %u is already in the neuron map\n", node_id);
    throw SRE((string) buf);
  }
  ...
  n->dendritic_couplings = dendritic_couplings;
  n->dendritic_thresholds = dendritic_thresholds;

  n->dendritic_charges = vector<double>(num_dendrites, 0.0);
...
Synapse* Network::add_synpase(uint32_t from_id, uint32_t to_id, double weight, uint32_t delay, uint32_t branch) 
{
  Neuron *from, *to;
  Synapse *syn;
  unordered_map <uint32_t, Neuron*>::const_iterator it;
  ...
  if (!is_neuron(from_id)) {
    snprintf(buf, 200, "drisp::Network::add_synpase() - node %u does not exist", from_id);
    throw SRE((string) buf);
  }
  if (!is_neuron(to_id)) {
    snprintf(buf, 200, "drisp::Network::add_synpase() - node %u does not exist", to_id);
    throw SRE((string) buf);
  }
  ...
  syn = new Synapse(weight, delay, to, branch);
...
void Network::add_input(uint32_t node_id, int input_id) 
{
  ...
  if (!is_neuron(node_id)) {
    snprintf(buf, 200, "drisp::Network::add_input() - node %u does not exist", node_id);
    throw SRE((string) buf);
  }
  if (input_id < 0) throw SRE("drisp::Network::add_input() - input_id < 0");
...
void Network::add_output(uint32_t node_id, int output_id) 
{
  char buf[200];
  if (!is_neuron(node_id)) {
    snprintf(buf, 200, "drisp::Network::add_output() - node %u does not exist", node_id);
    throw SRE((string) buf);
  }
  if (output_id < 0) throw SRE("drisp::Network::add_output() - output_id < 0");
...
Neuron* Network::get_neuron(uint32_t node_id) 
{
  ...
  if (it == neuron_map.end()) {
    snprintf(buf, 200, "drisp::Network::get_neuron() - %u is not in the neuron map\n", node_id);
    throw SRE((string) buf);
  }
  return it->second;
}
...
void Network::process_events(uint32_t time)
{
  ...
  */
  const vector<Event> es = std::move(events[time]);
  ...
  for (i = 0; i < es.size(); i++) {
    ...
    if (n->leak) {
      ...
      /*
        reset dendritic charges
        (LW): THIS IS A DESIGN CHOICE
      */
      for (j = 0; j < n->dendritic_charges.size(); j++) {
        n->dendritic_charges[j] = 0;
      }
    }
    ...
    /*
      enforce minimum dendritic charges
      (LW): THIS IS A DESIGN CHOICE
    */
    for (j = 0; j < n->dendritic_charges.size(); j++) {
      if (n->dendritic_charges[j] < min_potential) {
        n->dendritic_charges[j] = min_potential;
      }
    }
  }
  ...
  /*
    ...
    First, accumulate charges for each dendritic branch
      - check what branch each incoming synapse belongs to
      - accumulate the charges for each branch

    Then, threshold on dendritic branches
      - Check if each branch has exceeded its threshold

    Accumulate the charges for soma based on dendritic couplings
      - accumulate couplings for each branch that fired
  */

  /* accumulate dendritic charges */

  for (i = 0; i < es.size(); i++) {
    n = es[i].n;
    n->check = true;

    // For each branch, accumulate the charge
    n->dendritic_charges[es[i].b] += es[i].c;
  ...
  /* threshold on dendritic branches */
    ...
    if (n->check == true) {
      ...
      if (n->charge >= n->threshold) {
        ...
        for (j = 0; j < n->synapses.size(); j++) {
          ...
          events[to_time].push_back(Event(syn->to, weight, syn->branch));
...
Processor::Processor(json &params) {
  ...
  if (params.contains("noisy_weights")) {
    estring =  "DRISP: noisy_weights is no longer supported.\n";
    estring += "      Instead, you simply set the stds array, and it will be used.\n";
    estring += "      Please also see utils/drisp_08_2024.\n";
    throw SRE(estring);
  }

  Parameter_Check_Json_T(params, drisp_spec);
  ...
  if (params.contains("weights") && params["weights"].size() > 0) {
    ...
    if (params.contains("min_weight")) throw SRE("DRISP: Cannot have weights and min_weight.");
    if (params.contains("max_weight")) throw SRE("DRISP: Cannot have weights and max_weight.");
    if (!params.contains("inputs_from_weights")) {
      throw SRE("DRISP: If you specify weights, then you must specify inputs_from_weights.");
    }

    for (i = 1; i < weights.size(); i++) {
      if (weights[i] < weights[i-1]) {
        snprintf(buf, 100, "(%lg and %lg)", weights[i-1], weights[i]);
        throw SRE((string) "DRISP: If you specify weights, then they must be sorted. " + buf);
      }
      if (discrete && weights[i] - ((int) weights[i]) != 0) {
        throw SRE("DRISP: Since discrete is true, weights must be integers.\n");
      }
    }
    ...
    if (!inputs_from_weights && !params.contains("spike_value_factor")) {
      throw SRE("DRISP: If inputs_from_weights is false, you must specify spike_value_factor.");
    }

    if (inputs_from_weights && params.contains("spike_value_factor")) {
      throw SRE("DRISP: If inputs_from_weights is true, you cannot specify spike_value_factor.");
    }
    ...
    if (!params.contains("min_weight")) throw SRE("DRISP: Need parameter min_weight.");
    if (!params.contains("max_weight")) throw SRE("DRISP: Need parameter max_weight.");
    if (params.contains("inputs_from_weights")) {
      throw SRE("DRISP: If you don't specify weights, you cannot specify inputs_from_weights.");
    }
...
Processor::~Processor() {
  map <int, drisp::Network*>::const_iterator it;
...
bool Processor::load_network(neuro::Network* net, int network_id) {
  ...
  drisp::Network *drisp_net;
  string error;
  string rln = "drisp::load_network() - ";
  ...
  drisp_net = new drisp::Network(net, 
                               spike_value_factor,
                               min_potential,
                               leak_mode[0], 
                               run_time_inclusive, 
...
void Processor::clear(int network_id) {
  drisp::Network *drisp_net = get_drisp_network(network_id);
...
void Processor::apply_spike(const Spike& s, bool normalized, int network_id) {
  get_drisp_network(network_id)->apply_spike(s, normalized);
}
...
void Processor::apply_spikes(const vector<Spike>& s, bool normalized, int network_id) {
  size_t i;
  drisp::Network *drisp_net = get_drisp_network(network_id);

  for (i = 0; i < s.size(); i++) {
    drisp_net->apply_spike(s[i], normalized);
  }
}
...
void Processor::apply_spikes(const vector<Spike>& s, 
  ...
  size_t i;
  for (i = 0; i < network_ids.size(); i++) {
    apply_spikes(s, normalized, network_ids[i]);
  }
}
...
void Processor::run(double duration, int network_id) {
  get_drisp_network(network_id)->run(duration);
}
...
void Processor::run(double duration, const vector<int>& network_ids) {
  size_t i;
  for (i = 0; i < network_ids.size(); i++) {
    get_drisp_network(network_ids[i])->run(duration);
  }
}
...
double Processor::get_time(int network_id) {
  return get_drisp_network(network_id)->get_time();
}



bool Processor::track_output_events(int output_id, bool track, int network_id) {
  return get_drisp_network(network_id)->track_output_events(output_id, track);
}



bool Processor::track_neuron_events(uint32_t node_id, bool track, int network_id) {
  return get_drisp_network(network_id)->track_neuron_events(node_id, track);
}



double Processor::output_last_fire(int output_id, int network_id) {
  return get_drisp_network(network_id)->output_last_fire(output_id);
}



vector <double> Processor::output_last_fires(int network_id) {
  return get_drisp_network(network_id)->output_last_fires();
}



int Processor::output_count(int output_id, int network_id) {
  return get_drisp_network(network_id)->output_count(output_id);
}



vector <int> Processor::output_counts(int network_id) {
  return get_drisp_network(network_id)->output_counts();
}



vector <double> Processor::output_vector(int output_id, int network_id) {
  return get_drisp_network(network_id)->output_vector(output_id);
}



vector < vector <double> > Processor::output_vectors(int network_id) {
  return get_drisp_network(network_id)->output_vectors();
}



long long Processor::total_neuron_counts(int network_id) {
  return get_drisp_network(network_id)->total_neuron_counts();
}



long long Processor::total_neuron_accumulates(int network_id) {
  return get_drisp_network(network_id)->total_neuron_accumulates();
}



vector <int> Processor::neuron_counts(int network_id) {
  return get_drisp_network(network_id)->neuron_counts();
}



vector <double> Processor::neuron_last_fires(int network_id) {
  return get_drisp_network(network_id)->neuron_last_fires();
}



vector < vector <double> > Processor::neuron_vectors(int network_id) {
  return get_drisp_network(network_id)->neuron_vectors();
}



vector < double > Processor::neuron_charges(int network_id) {
  return get_drisp_network(network_id)->neuron_charges();
}
...
void  Processor::synapse_weights(vector <uint32_t> &pres,
                               ...
                               vector <double> &vals,
                               int network_id) {
  return get_drisp_network(network_id)->synapse_weights(pres, posts, vals);
}
...
void Processor::clear_activity(int network_id) {
  get_drisp_network(network_id)->clear_activity();
}
...
PropertyPack Processor::get_network_properties() const {
  ...
  pp.add_edge_property("D_Branch", 0, num_dendrites - 1, Property::Type::INTEGER);
...
Network* Processor::get_drisp_network(int network_id) {
  map <int, drisp::Network*>::const_iterator it;
  char buf[200];
  it = networks.find(network_id);
  if (it == networks.end()) {
    snprintf(buf, 200, "drisp::Processor::get_drisp_network() network_id %d does not exist", 
             network_id);
    throw SRE((string) buf);
  }
  return it->second;
