...
struct Event {
  ...
  Event() : n(nullptr), c(0.0), b(0) {}

  Event(Neuron* n, double c, uint32_t b)
      : n(n), c(c), b(b) {}
...
public:
  ...
  /* Dendritic variables */
  vector <double> dendritic_couplings;   /**< Dendritic branch coupling with parent soma */
  vector <double> dendritic_thresholds;
  vector <double> dendritic_charges;
};
...
class Synapse {
public:
  Synapse(double w, uint32_t d, Neuron* to_n, uint32_t b);
  double weight;             /**< Weight value */
  Neuron *to;                /**< To neuron on this synapse */
  uint32_t delay;            /**< Delay value */

  /* Dendritic variables */
  uint32_t branch;           /**< branch id to connect to */
};
...
public:
  ...
  /** convert network in framework format to an internal drisp network. */
...
protected:
  Neuron* add_neuron(uint32_t node_id, double threshold, bool leak, const vector<double>& dendritic_couplings, const vector<double>& dendritic_thresholds);
  Synapse* add_synpase(uint32_t from_id, uint32_t to_id, double weight, uint32_t delay, uint32_t branch);
   ...
   * Additionally, events specify what dendritic branch each event belongs to (int).
   */
  ...
  bool fire_like_ravens;    /**< Register neuron fires one step later, so DRISP can match RAVENS. */
...
protected:
  ...
  drisp::Network* get_drisp_network(int network_id);
  double get_input_spike_factor() const;
  map <int, drisp::Network*> networks;
